...
export function SparsityFrontierPlot({ sparsities, gapResults, errorResults }: PlotProps) {
  ...
  const downloadChart = () => {
    if (chartRef.current) {
       htmlToImage.toPng(chartRef.current, { backgroundColor: '#020617' })
         .then(dataUrl => {
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={gapData} margin={{ top: 10, right: 10, left: -10, bottom: 25 }}>
                ...
                <XAxis 
                  ...
                  label={{ value: 'KV Cache Reads (%)', position: 'insideBottom', offset: -15, fill: '#9CA3AF', fontSize: 12 }}
                />
                <YAxis 
                  stroke="#9CA3AF" 
                  domain={['auto', 102]} 
                  tick={{ fill: '#9CA3AF', fontSize: 12 }}
                  label={{ value: 'Accuracy (%)', angle: -90, position: 'insideLeft', fill: '#9CA3AF', dy: 50, fontSize: 12 }}
                />
                <Tooltip 
                ...
                />
                <Legend verticalAlign="top" wrapperStyle={{ fontSize: '0.75rem', paddingBottom: '10px' }} iconType="circle" />
                <ReferenceLine y={100} stroke="#525252" strokeDasharray="3 3" label={{ value: 'Dense', fill: '#737373', fontSize: 10 }} />
                ...
                {TARGET_BASELINES.map(target => (
                  <Line
                    ...
                    dot={{ r: 4, fill: target.color, strokeWidth: 0 }}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={errorData} margin={{ top: 10, right: 10, left: -10, bottom: 25 }}>
                ...
                <YAxis 
                  ...
                  domain={[0, 'auto']} 
                  tick={{ fill: '#9CA3AF', fontSize: 12 }}
                  label={{ value: 'Error', angle: -90, position: 'insideLeft', fill: '#9CA3AF', dy: 30, fontSize: 12 }}
                />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1a1a1a', border: '1px solid #333333', borderRadius: '0.5rem' }}
                  itemStyle={{ fontSize: '0.85rem' }}
